...
- `kavalai/demo_agents/`, `kavalai/demo_tasks/`, `kavalai/demo_personas/`: Sample configurations and data.
        ...
        - `workflow_model.py`: Data models for workflows, including `WorkflowModel`, `Task`, `RestServer`, and `TypeInputInfo`. All these models now inherit from `YamlModel`, which preserves `_line_number` and `_file_path` as private attributes to facilitate precise error reporting from the original YAML source. Includes `to_plain` utility for recursive Pydantic model serialization. Supports `temperature` parameter at both workflow and task levels (including `AgentTask` for Planning Agent, default: `None`, validated to be between 0.0 and 2.0). TypeInputInfo now supports a third type: `load_from_history` to pull values from previous runs' contexts. `Task` now includes `max_steps`, `allowed_mcp_servers`, `timeout` for planning agent orchestration, and a `name` field for identification. Added `RagQueryTask` for RAG-based search within workflows. Tasks now use `stream_updates` (for task progress) and `stream_output` (for result content) instead of a single `stream` boolean.
