...
- `kavalai/`: Main Python package.
    - `agents/`: SDK and agent runtime logic.
        ...
        - `workflow.py`: Core workflow engine (YAML to execution). Supports case-insensitive (snake_case conversion) loading of tasks and workflow models from YAML. `RunContext.resolve_context_value` supports nested dotted paths (e.g., `input.criteria.keywords`) and a `.length` suffix for collections. `RunContext.prepare_tool_inputs` always returns a dictionary of inputs, ensuring Pydantic models are dumped to dicts. `RunContext.resolve_input_info` resolves `TypeInputInfo` to values, with default path resolution (using dictionary keys) handled by callers like `prepare_tool_inputs` or `run_prompt`. Supports conditional task execution via the `when` field in tasks, with operators (`eq`, `not_eq`, `gt`, `gte`, `lt`, `lte`, `contains`) and logical operators (`all`, `any`, `not`). Conditions are validated at initialization time (e.g., operator operand count). Tasks without `prompt` or `tool` are "combine" tasks, merging inputs into the specified output. REST servers support both `url` and `url_env` (with `url_env` taking precedence if defined in environment) and use `username_env` and `password_env` for basic auth. Tasks can specify an HTTP `method` for REST tool calls. Supports streaming output for both prompts (incremental) and tool/combine tasks (completion only) via an `asyncio.Queue` passed as the `queue` parameter to `run()`.
        - `workflow_model.py`: Data models for workflows, including `WorkflowModel`, `Task`, `RestServer`, and `TypeInputInfo`. Includes `to_plain` utility for recursive Pydantic model serialization. Supports `temperature` parameter at both workflow and task levels (default: 0.0, validated to be between 0.0 and 2.0). TypeInputInfo now supports a third type: `load_from_history` to pull values from previous runs' contexts.
        - `workflow_validation.py`: Dedicated module for workflow validation logic, including REST server environment variables, temperature ranges (0.0 to 2.0), and overall workflow structure.
