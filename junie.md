...
- `kavalai/demo_agents/`, `kavalai/demo_tasks/`, `kavalai/demo_personas/`: Sample configurations and data.
    ...
    - `llm_clients/`: Native LLM client implementations.
        - `base_client.py`: Base class `BaseLlmClient` that handles the core `chat_completions` logic, including `Streamer` initialization and background task creation with `with_retry`. Subclasses should override `_run_chat_completions`.
        - `common.py`: Shared LLM client utilities, including `ModelCallStat` creation, embedding normalization, and `safe_parse_json`.
        - `streamer.py`: `Streamer` helper class to manage and push streaming content to an asyncio queue, including `StreamContent` model. Supports buffering, delta/non-delta streaming, automated JSON parsing for structured outputs, and optional timeouts with tracking of active streamers.
    - `normalizer.py`: Embedding transformation utilities including L1/L2 normalization and centering. Supports server-side centering parameter learning from RAG index (using PostgreSQL `avg()`), YAML saving/loading (from files or strings), and numpy-optimized batch operations.
        ...
        - `v2/openai_client.py`: Native OpenAI client implementation (v2) using the Responses API and Streamer. Inherits from `BaseLlmClient` and returns a `Streamer`. Now supports model call statistics collection and reporting via `ModelStatsReceiver`.
        - `v2/gemini_client.py`: Native Gemini client implementation (v2) using the Streamer. Inherits from `BaseLlmClient` and returns a `Streamer`. Now supports model call statistics collection and reporting via `ModelStatsReceiver`.
        - `v2/ollama_client.py`: Native Ollama client implementation (v2) using the Streamer. Inherits from `BaseLlmClient` and returns a `Streamer`. Now supports model call statistics collection and reporting via `ModelStatsReceiver`.
