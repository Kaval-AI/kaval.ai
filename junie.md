...
- `kavalai/`: Main Python package.
    - `agents/`: SDK and agent runtime logic.
        ...
        - `schema_parser.py`: Pydantic model generation from JSON schemas for input/output validation. Generated models use `extra='forbid'` (ConfigDict) to ensure `additionalProperties: false` in JSON schema, which is required for OpenAI Structured Outputs. Supports inline object definitions by creating nested Pydantic models. Validates schema attributes, raising `ValueError` for unknown keys (e.g., misspelled attributes like `maxLength`). Supports optional fields via `required: false` and optional default values via `default: value` in property definitions.
