...
- `kavalai/demo_agents/`, `kavalai/demo_tasks/`, `kavalai/demo_personas/`: Sample configurations and data.
        ...
        - `sessions.py`: Service for retrieving session summaries and messages. `get_sessions_summary` supports pagination and filtering by `agent_id`, `search` string (substring search in chat messages), `start_date`, and `end_date` (filter by session creation time).
