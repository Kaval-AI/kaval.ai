...
- `kavalai/`: Main Python package.
    - `agents/`: SDK and agent runtime logic.
        ...
        - `rag_service.py`: Service for indexing and querying text using embeddings (RAG). `query` now returns a list of `RagServiceResult` Pydantic models and supports `keep_best` parameter to return only the best match per `source_id`.
