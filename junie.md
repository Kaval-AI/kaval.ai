...
- `kavalai/`: Main Python package.
    ...
    - `normalizer.py`: Embedding transformation utilities including L1/L2 normalization and centering. Supports server-side centering parameter learning from RAG index (using PostgreSQL `avg()`), YAML saving/loading (from files or strings), and numpy-optimized batch operations.
        - `llm_client.py`: High-level LLM client interface. Includes `chat_completions` for executing LLM calls with comprehensive metric collection (tokens, duration, request/response data, cost), `compute_embeddings` for generating text embeddings, and `generate_image` for generating images from prompts. Both return a result/stats tuple, letting the caller decide whether to log stats using `_save_model_stats`. Refactored to use native OpenAI and Gemini clients instead of instructor for better structured output and stats collection. Includes `get_llm_client` factory. Supports configurable timeouts via `KAVALAI_LLM_TIMEOUT` (default: 30s). Added streaming support to `chat_completions`.
        - `openai_client.py`: Native OpenAI client wrapper. Supports structured outputs via `beta.chat.completions.parse`, streaming via `beta.chat.completions.stream`, and image generation via `images.generate`. Both `chat_completion`, `compute_embeddings`, and `generate_image` return a tuple of `(result, ModelCallStat)`. `chat_completion` now includes retry logic (up to 3 attempts) for `LengthFinishReasonError`. Supports `service_tier` parameter (auto, default, flex, priority) for chat completion calls. Handles empty stream buffers and missing usage data safely.
        - `gemini_client.py`: Native Gemini client wrapper. Supports structured outputs via `response_schema` (with schema cleanup for compatibility), streaming via `generate_content_stream`, and image generation via `generate_image`. Both `chat_completions`, `compute_embeddings`, and `generate_image` return a tuple of `(result, ModelCallStat)`.
...
- `docs/`: Sphinx documentation.
    ...
    - `conf.py`: Sphinx configuration (includes `autodoc`, `napoleon`, `viewcode`, `todo`, and `sphinx_immaterial`).
