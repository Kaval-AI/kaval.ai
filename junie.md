...
- `kavalai/functionkernel.py`: Manages tool registration and execution (REST, MCP, Python), providing a unified interface for agent components. It ensures no name conflicts exist between registered tools and servers by raising a `WorkflowException` on duplicates. It can generate unified tool descriptions for LLM prompts using strictly the `protocol://[name|module].function_name(args: type) -> return_type` format. Supports `register_rest_tool` to register individual REST endpoints with specific HTTP methods, input/output models, and descriptions. `ToolDefinition.__str__` prints a description in JSON format (name, description, inputSchema) similar to `get_tool_descriptions()`. `get_tool_descriptions()` is an async method that provides clear instructions, an example `ToolCall`, and concise input/output JSON schemas (recursively expanding nested Pydantic models using `model_json_schema()`) for all registered tools to help LLMs correctly format `ToolCall` arguments. It supports an `allowed_tools` filter (must be explicitly provided; if `None` or empty, no tools are returned) to control which tools are listed. Python functions used as tools must be decorated with `@kavalai.pythontool` and explicitly registered via `register_python_tool` to be called, enhancing security. Only docstrings (parsed using `inspect.getdoc()`) are used for Python tool descriptions. Features a refactored architecture with a dedicated `Validator` class for tool URI parsing, argument validation, and result casting to ensure high readability and maintainability.
- `tests/`: Comprehensive backend test suite. `tests/test_functionkernel.py` provides 96%+ unit test coverage for `kavalai/functionkernel.py` without using broad mocks, covering REST, MCP, and Python tool registration and execution. It uses a robust wait-for-server mechanism for SSE tests to avoid flaky connection timeouts. `tests/agents/test_rag_service.py` provides 100% test coverage for `kavalai/agents/rag_service.py`. `tests/llm_clients/test_streamer.py` provides 100% test coverage for `kavalai/llm_clients/streamer.py`. Mock MCP servers used in tests are located in `tests/helpers/`.
...
- `kavalai/demo_agents/`, `kavalai/demo_tasks/`, `kavalai/demo_personas/`: Sample configurations and data.
        ...
        - `workflow.py`: Core workflow engine (YAML to execution). Supports case-insensitive (snake_case conversion) loading of tasks and workflow models from YAML. Employs `LineLoader` (based on `PyYAML`) to capture and preserve exact line number metadata for every workflow component. Features enhanced error reporting via `format_yaml_error`, providing rich contextual snippets with pointers to the specific lines where validation or runtime exceptions occur. Supports conditional task execution via the `when` field in tasks, with operators (`eq`, `not_eq`, `gt`, `gte`, `lt`, `lte`, `contains`, `is_null`, `is_not_null`, `is_true`, `len`) and logical operators (`all`, `any`, `not`). Tasks can also have a `stop` boolean field; if true, the workflow stops after executing that task. Tasks without `prompt` or `tool` are "combine" tasks, merging inputs into the specified output. REST servers support both `url` and `url_env` (with `url_env` taking precedence if defined in environment) and use `username_env` and `password_env` for basic auth. Tasks can specify an HTTP `method` for REST tool calls. Supports streaming output for both prompts (incremental) and tool/combine tasks (completion only) via an `asyncio.Queue` passed as the `queue` parameter to `run()`. Now also supports MCP (Model Context Protocol) tool calls via stdio and SSE (HTTP) clients; define servers in `mcp_servers` and reference them in tasks via `mcp_server`. MCP client sessions are initialized on the first tool call of a run and reused for subsequent calls on the same server until the run completes. The tool name is provided in `tool`, and inputs are passed as arguments to MCP tools. MCP servers support `command`, `command_env`, `url`, and `url_env`. Now supports `python_tool` for calling Python functions directly; specify the full function path (e.g., `mypackage.mymodule.myfunction`) in `python_tool`. The runtime uses `inspect` to validate that inputs match the function signature and supports both sync and async functions. Added `run_rag_task` to perform RAG-based search within workflows, supporting text resolution from context and storing results in the run data. It now also saves tool call information in the database. Added `run_planning_agent` to invoke the `PlanningAgent` for complex multi-step tasks, with support for chat history and streaming.
        - `run_context.py`: `RunContext` model and helpers. `resolve_context_value` supports nested dotted paths (e.g., `input.criteria.keywords`) and a special `.length` segment for collections. `prepare_tool_inputs` always returns a dictionary of inputs, ensuring Pydantic models are dumped to dicts. `resolve_input_info` resolves `TypeInputInfo` to values, including `load_from_history` when `agent_service` and `session_id` are set. `evaluate_condition` supports comparison operators (`eq`, `not_eq`, `gt`, `gte`, `lt`, `lte`, `contains`, `is_null`, `is_not_null`, `is_true`, `len`) and logical combinators (`all`, `any`, `not`) with short-circuiting for `all` and `any`. `render_prompt` supports `{{ templates.NAME }}`, `{{ context.PATH }}`, and `{{ history.PATH }}` syntax, raising `ValueError` with descriptive messages if resolution fails.
        - `workflow_model.py`: Data models for workflows, including `WorkflowModel`, `Task`, `RestServer`, and `TypeInputInfo`. All these models now inherit from `YamlModel`, which preserves `_line_number` and `_file_path` as private attributes to facilitate precise error reporting from the original YAML source. `YamlModel` is configured to forbid extra fields. Includes `to_plain` utility for recursive Pydantic model serialization. Supports `temperature` and `llm_model` parameters at both workflow and task levels (including `AgentTask` for Planning Agent, default: `None`, temperature validated to be between 0.0 and 2.0). Now supports `llm_kwargs` at both workflow and task levels (dictionary of arbitrary parameters passed to the LLM client, where task-level values override workflow-level ones). TypeInputInfo now supports a third type: `load_from_history` to pull values from previous runs' contexts. `Task` now includes `max_steps`, `allowed_mcp_servers`, `timeout` for planning agent orchestration, and a `name` field for identification. Added `RagQueryTask` for RAG-based search within workflows. Tasks now use `stream_updates` (for task progress) and `stream_output` (for result content) instead of a single `stream` boolean. The `stream` field is explicitly checked and rejected with a deprecation error. `AgentTask` now supports `stream_persisted` to stream the complete value of a tool result if `persist_to` is used. Updated `Workflow` to initialize `Streamer` if any of `stream_output`, `stream_persisted`, or `stream_updates` are enabled.
        - `utils.py`: Utility functions including `clean_text` for string normalization (NFC) and removal of null characters and control characters.
        - `workflow_validation.py`: Dedicated module for workflow validation logic. It leverages component metadata (line numbers and file paths) to provide pinpoint accuracy in reporting errors such as invalid REST server environment variables, out-of-range temperature settings (0.0 to 2.0), and structural workflow inconsistencies. Validates `allowed_mcp_servers` in tasks.
        - `planning_agent.py`: Planning agent that orchestrates multi-step tool usage (REST + MCP) using an LLM planner. Handles tool calling, looping up to `max_steps`, and stores tool results in `planner_context`. The `run` method now requires a `task_name` parameter for better identification and logging. It provides a structured `StepOutput` per iteration, which includes explanations, tool calls, and final results. To ensure compatibility with OpenAI's Structured Outputs, `StepOutput` and `ToolCall` models strictly forbid extra properties and enforce `additionalProperties: false` in their JSON schemas. The system prompt is composed from multiple pieces (task, instructions, tools, inputs, context, etc.) joined together, with Pydantic models in `planner_context` and `step_outputs` being explicitly dumped to JSON. Supports optional `streamer` for real-time output and final completion streaming. Now logs tool calls, short explanations, and all intermediate steps as separate tasks named `{task_name}_step_{idx}`. Fixed compatibility with Gemini by ensuring at least one "user" message is sent. Improved `short_explanation` field description to encourage concise output within 50-character limit. Now gracefully handles tool execution errors by capturing the exception and passing it back to the planner in the context, allowing for self-correction; these errors are logged as `ERROR` for better visibility and recorded in the `errors` list of the task. Multiple tool calls in a single step are executed in parallel using `asyncio.gather`. Support for `persist_to` in `ToolCall` allows storing tool results directly into `run_context.data`. Tool task records are added to `agent_service` if available, with robust error logging for serialization issues. Simplified tool calling by categorizing arguments into `literal_args`, `planner_context_args`, and `input_args` (all as JSON strings), which are merged before execution while ensuring no duplicate keys exist. Now accepts and uses `llm_kwargs` for LLM calls. Now automatically generates deterministic `call_id` (format: `tool_call_{iter_no}_{idx}`) for tool calls that lack them.
        - `v2/agent.py`: Refactored version of the Planning Agent using v2 LLM clients. Features a modular architecture with a dedicated `AgentTemplater` class and clean, distinct functions for prompt building, tool execution, and LLM interaction. Bridges v2 LLM streamers to the project's common streamer for consistent real-time updates across providers.
        - `server.py`: REST server for agents. Supports optional HTTP basic authentication. Logs masked database connection details (using `***`), basic auth configuration status, database pooling settings, and OpenAI service tier upon startup. Includes `mask_db_uri` for secure logging of database URIs. Added `/liveness` and `/health` endpoints for K8s probes; `/health` performs a database connectivity check. `create_app_from_env_conf` supports optional parameters to override environment variables. `create_agent_app` stores the workflow in `app.state.workflow`. Supports `/run_agent` for synchronous execution and `/stream_agent` for streaming output (SSE). `handle_agent_stream` now supports an optional `output_streamer_name` parameter (default: "run_output") for the final result event. `stream_agent` also streams this final event containing the full result (session ID and data).
        - `sessions.py`: Service for retrieving session summaries and messages. `get_sessions_summary` supports pagination and filtering by `agent_id`, `search` string (substring search in chat messages), `start_date`, and `end_date` (filter by session creation time). Includes `errors_count` in session summaries. Added `get_session_details` to fetch full session history including grouped messages, runs with task counts, and task details (including `name`, `prompt`, `errors`, and `duration_seconds`).
        ...
        - `agent_service.py`: Service for managing agent state, sessions, runs, LLM profiles, and embedding profiles in the DB. `AgentService` is initialized with an `async_sessionmaker` to efficiently manage database connections from a pool. `get_or_create_agent` updates existing agents if description, schemas or workflow changed. Now includes `total_cost` in LLM and embedding profile views. `get_history_value` retrieves the most recent value from previous runs' context for a given key or dotted path, optimized with root-level key check in the SQL query. Includes `add_model_call_stats` for unified recording of LLM and embedding metrics.
        - `rag_service.py`: Service for indexing and querying text using embeddings (RAG). `RagService` is initialized with an `async_sessionmaker` or a session factory. Includes class methods `from_uri` and `from_session_maker` for flexible instantiation. `query` and `batch_query_with_join` now return a list of results and support `keep_best` parameter to return only the best match per `source_id`. Now supports `source_ids` parameter to filter results. Added `batch_query` to query multiple texts in a single database call using PostgreSQL `CROSS JOIN LATERAL` and `WITH ORDINALITY` for efficient, ordered results. Added `compute_similarity_matrix` to compute similarities between multiple texts and source IDs in a single query. Added `learn_normalizer` to learn centering and normalization parameters from the index. `RagService` now accepts an optional `normalizer` instance in `__init__` which is used for all embedding computations. `tests/agents/test_workflow_rag.py` provides coverage for RAG tasks in workflows, including source filtering and metadata handling.
    - `llm_clients/`: Native LLM client implementations.
        - `base_client.py`: Base class `BaseLlmClient` that handles the core `chat_completions` logic, including `Streamer` initialization and background task creation with `with_retry`. Background tasks are wrapped in error handling to propagate exceptions to the `Streamer`. Subclasses should override `_run_chat_completions`. Includes `ModelStatsLogger` for configurable logging of model statistics.
        - `common.py`: Shared LLM client utilities, including `ModelCallStat` creation, embedding normalization, and `safe_parse_json`.
        - `streamer.py`: `Streamer` helper class to manage and push streaming content to an asyncio queue, including `StreamContent` model. Supports buffering, delta/non-delta streaming, automated JSON parsing for structured outputs, optional timeouts with tracking of active streamers, and explicit error signaling via `stream_error`.
    - `normalizer.py`: Embedding transformation utilities including L1/L2 normalization and centering. Supports server-side centering parameter learning from RAG index (using PostgreSQL `avg()`), YAML saving/loading (from files or strings), and numpy-optimized batch operations.
        ...
        - `openai_client.py`: Native OpenAI client wrapper. Supports structured outputs via `beta.chat.completions.parse` and streaming via `beta.chat.completions.stream`. Both `chat_completions` and `compute_embeddings` return a tuple of `(result, ModelCallStat)`. `chat_completions` handles images for multimodal calls and supports `stream_delta` (default: False) to stream only incremental updates. `compute_embeddings` supports normalization and custom normalizers. Supports default timeout in `__init__` and per-call overrides. Supports `service_tier` parameter.
        - `v2/openai_client.py`: Native OpenAI client implementation (v2) using the Responses API and Streamer. Inherits from `BaseLlmClient`. `stream_chat_completions` returns a `Streamer`, while `chat_completions` returns the final result. Now supports model call statistics collection and reporting via `ModelStatsReceiver`.
        - `v2/gemini_client.py`: Native Gemini client implementation (v2) using the Streamer. Inherits from `BaseLlmClient`. `stream_chat_completions` returns a `Streamer`. Now supports model call statistics collection and reporting via `ModelStatsReceiver`.
        - `v2/ollama_client.py`: Native Ollama client implementation (v2) using the Streamer. Inherits from `BaseLlmClient`. `stream_chat_completions` returns a `Streamer`. Now supports model call statistics collection and reporting via `ModelStatsReceiver`.
        - `gemini_client.py`: Native Gemini client wrapper. Supports structured outputs via Pydantic models passed directly to `response_schema`. Both `chat_completions` and `compute_embeddings` return a tuple of `(result, ModelCallStat)`. Now always uses streaming in `chat_completions` for both streaming and non-streaming requests, unifying text and structured output handling. Now supports `provider/model` syntax by automatically extracting the model part. Uses `GEMINI_API_KEY` from environment. Supports reasoning/thinking parameters (`reasoning_effort`, `thinking_level`, `thinking_budget`) for models that support it. Correctly handles system instructions by extracting them from messages. Now streams thoughts in real-time when `thinking_budget` is used. Supports `stream_delta` (default: False) to stream only incremental updates for both text and thoughts. Includes module-level utility functions for message conversion and config preparation. Supports default timeout in `__init__` and per-call overrides. Now handles `service_tier` by popping it from config to avoid SDK validation errors while logging the request.
        - `ollama_client.py`: Native Ollama client wrapper using the `ollama` Python library. Supports `chat_completions` with optional structured output (via `format: json`) and streaming. Supports `compute_embeddings` and `list_models`. Handles `OLLAMA_HOST` from environment (default: `http://localhost:11434`).
        - `fastembed_client.py`: FastEmbed client for generating embeddings locally using ONNX Runtime. Supports `compute_embeddings` and `list_models`. Handles `FASTEMBED_CACHE_DIR` and `FASTEMBED_THREADS` from environment.
        - `kwargs_mapper.py`: `LLMKWargsMapper` that converts common YAML `llm_kwargs` (e.g., `reasoning`, `priority`, `max_tokens`, unified `stop/stop_sequences`) into provider-specific parameters for OpenAI, Gemini, and Ollama, used automatically by `LLMClient.chat_completions`.
...
- `backoffice/`: API and logic for the management UI.
        - `__init__.py`: Package initialization, defines `PACKAGE_PATH` and `MIGRATIONS_PATH`.
        - `server.py`: FastAPI server for the backoffice API. Includes endpoints for agents, sessions, stats, projects (including membership management), and workflow visualization. `projects_rag_query` supports filtering by `source_ids` and optional `normalizer_yaml`. `projects_train_pca` triggers PCA training and streams progress via SSE. Now handles both backoffice and project database connection errors gracefully by returning a 503 Service Unavailable error with a descriptive message.
...
- **Frontend**: Angular (using modern built-in control flow: `@if`, `@for`). All components use the modern `@if` and `@for` syntax; deprecated `*ngIf` and `*ngFor` are not used.
