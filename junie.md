...
- `kavalai/demo_agents/`, `kavalai/demo_tasks/`, `kavalai/demo_personas/`: Sample configurations and data.
        ...
        - `workflow_model.py`: Data models for workflows, including `WorkflowModel`, `Task`, `RestServer`, and `TypeInputInfo`. Includes `to_plain` utility for recursive Pydantic model serialization. Supports `temperature` parameter at both workflow and task levels (default: `None`, validated to be between 0.0 and 2.0). TypeInputInfo now supports a third type: `load_from_history` to pull values from previous runs' contexts. `Task` now includes `max_steps`, `allowed_mcp_servers`, `timeout` for planning agent orchestration, and a `name` field for identification. Added `RagQueryTask` for RAG-based search within workflows.
