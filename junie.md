...
- `kavalai/demo_agents/`, `kavalai/demo_tasks/`, `kavalai/demo_personas/`: Sample configurations and data.
    ...
    - `normalizer.py`: Embedding transformation utilities including L1/L2 normalization and centering. Supports server-side centering parameter learning from RAG index (using PostgreSQL `avg()`), YAML saving/loading (from files or strings), and numpy-optimized batch operations.
        ...
        - `v2/openai_client.py`: Native OpenAI client implementation (v2) using the Responses API and Streamer. Inherits from `BaseLlmClient` and returns a `Streamer`.
        - `v2/gemini_client.py`: Native Gemini client implementation (v2) using the Streamer. Inherits from `BaseLlmClient` and returns a `Streamer`.
        - `v2/ollama_client.py`: Native Ollama client implementation (v2) using the Streamer. Inherits from `BaseLlmClient` and returns a `Streamer`.
