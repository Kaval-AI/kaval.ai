...
- `kavalai/`: Main Python package.
    - `agents/`: SDK and agent runtime logic.
        ...
        - `workflow.py`: Core workflow execution engine (YAML to execution). REST servers use `username_env` and `password_env` to specify environment variable names for basic auth credentials (not literal values). Validates that env vars are defined when loading the workflow. Tasks can specify an HTTP `method` for REST tool calls (defaults to "get"). `run_tool` uses `RunContext.prepare_tool_inputs` to format tool inputs: a single input is passed as-is (e.g., for JSON bodies), while multiple inputs are formatted as a newline-separated string of `key:value` pairs.
        ...
        - `schema_parser.py`: Pydantic model generation from JSON schemas for input/output validation. Generated models use `extra='forbid'` (ConfigDict) to ensure `additionalProperties: false` in JSON schema, which is required for OpenAI Structured Outputs. Supports inline object definitions by creating nested Pydantic models.
