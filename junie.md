...
- `kavalai/demo_agents/`, `kavalai/demo_tasks/`, `kavalai/demo_personas/`: Sample configurations and data.
    ...
    - `llm_clients/`: Native LLM client implementations.
        - `base_client.py`: Base class `BaseLlmClient` that handles the core `chat_completions` logic, including `Streamer` initialization and background task creation with `with_retry`. Background tasks are wrapped in error handling to propagate exceptions to the `Streamer`. Subclasses should override `_run_chat_completions`.
