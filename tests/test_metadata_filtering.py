...
def test_to_plain_filters_metadata():
    data = {
        ...
        "nested": {"value": 1, "__line__": 11},
        "list": [{"item": 1, "__line__": 12}],
    }

    expected = {"name": "task", "nested": {"value": 1}, "list": [{"item": 1}]}
...
async def test_call_tool_strips_metadata():
    ...
    result = await kernel.call_tool(
        "python://my_tool",
        {"a": 1, "b": 2, "__line__": 99, "__file_path__": "test.yaml"},
    )
